...
- [ ] Prompt 20 — ProviderManager REAL (Base/Registry/Manager, Claude/OpenCode/Gemini/Local,
  failover, cooldown, token budgeting, persistence).
- [ ] Prompt 21 — ComplexityRoutingEngine (complexity scoring, cost/priority/context-aware,
  deterministic + auditable).

## Phase 4 — Workers
- [ ] Prompt 22 — OpenClawWorker REAL (consumer, sandbox exec, git branch-per-task, patching,
  test/lint, retry loops, artifacts). *Executes tasks; does not control workflows.*
- [ ] Prompt 23 — ClaudeArchitectWorker (architecture/review/spec/refactor handlers).
  *Expensive cognition layer, not execution.*

## Phase 5 — Autonomy
- [ ] Prompt 24 — AutonomousTaskChaining (dependency graph, auto task generation, repair/
  escalation/review/deploy tasks; bounded — no infinite loops).
- [ ] Prompt 25 — SelfHealingRuntime (failure analysis, stacktrace parsing, bounded repair
  loops, rollback, escalation).

## Phase 6 — CI/CD & release
- [ ] Prompt 26 — Full CI/CD: lint, pytest, mypy, security scan, Docker build validation,
  runtime smoke tests, release workflow, coverage, branch protection.
